        ...
        <div className="flex glass rounded-lg overflow-hidden">
          <button
            ...
            className={`px-4 py-1.5 text-sm font-medium transition-all duration-200 ${
              tab === "smart" ? "bg-cyan-500/15 text-cyan-400 shadow-glow-sm" : "text-gray-500 hover:text-cyan-300"
            }`}
          >
          ...
          <button
            onClick={() => setTab("kol")}
            className={`px-4 py-1.5 text-sm font-medium transition-all duration-200 ${
              tab === "kol" ? "bg-purple-500/15 text-purple-400" : "text-gray-500 hover:text-purple-300"
            }`}
          >
            KOLs ({kolList.length})
          </button>
      ...
      <div className="glass rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {list.map((t, i) => (
                <tr key={t.transaction_hash ?? i} className="border-t border-white/[0.04] hover:bg-cyan-500/[0.03] transition-colors duration-150">
                  ...
                  <td className="py-2.5 px-3">
                    ...
                    >
                      {t.base_token?.logo && (
                        <img src={t.base_token.logo} alt="" className="w-5 h-5 rounded-full bg-white/[0.05] ring-1 ring-white/[0.08]" loading="lazy" />
                      )}
                      <span className="font-medium text-sm">{t.base_token?.symbol ?? t.base_address?.slice(0, 8)}</span>
                      {t.base_token?.launchpad && (
                        <span className="text-[10px] text-gray-600 bg-white/[0.06] px-1.5 py-0.5 rounded-md">{t.base_token.launchpad}</span>
                      )}
                    </Link>
                  </td>
